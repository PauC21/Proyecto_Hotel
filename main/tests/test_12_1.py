...
class CrearReservaValidaTest(StaticLiveServerTestCase):
    """CP-RF-12-1: Crear una reserva válida desde el panel administrador"""
    ...
    def test_crear_reserva_valida(self):
        ...
        # 3️⃣ Llenar formulario de reserva
        ...
        self.assertEqual(reserva.status, "confirmed")

        print(f"✅ Reserva creada correctamente para {self.guest.first_name} "
              f"({self.room.room_number}) del {check_in} al {check_out}")
