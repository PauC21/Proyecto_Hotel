...
class EditarReservaTest(StaticLiveServerTestCase):
    """CP-RF-12-4: Editar reserva desde el panel administrador"""

    def setUp(self):
        chrome_options = Options()
        chrome_options.add_argument("--no-sandbox")
        chrome_options.add_argument("--disable-dev-shm-usage")
        # chrome_options.add_argument("--headless")  # quitar comment si no quieres ver navegador
    ...
    def test_editar_reserva(self):
        ...
        # 2️⃣ Ir a la edición de la reserva
        driver.get(f"{self.live_server_url}/admin/main/reservation/{self.reserva.id}/change/")
        self.take_screenshot("02_edit_reserva_page")

        # 3️⃣ Cambiar estado a un valor válido (primer opción disponible)
        status_select = Select(driver.find_element(By.NAME, "status"))
        first_option_value = status_select.options[0].get_attribute("value")
        status_select.select_by_value(first_option_value)
        self.take_screenshot("03_status_changed")
        ...
        self.assertEqual(self.reserva.status, first_option_value)
        print(f"✅ Reserva editada correctamente, nuevo estado: {self.reserva.status}")
